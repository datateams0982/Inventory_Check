...
def price_volume_trend(df, rolling_window):
    ...
    df_pvt['pvt_current'] = df_pvt['vol'] * (df_pvt['close'] - df_pvt['close_lag']) / df_pvt['close_lag']
    ...
    for buyer in ['foreign_', 'investment_', 'dealer_']:
        df_pvt[f'{buyer}pvt_current'] = df_pvt[f'{buyer}buy'] * (df_pvt['close'] - df_pvt['close_lag']) / df_pvt['close_lag']
...
def log_return(row, problem='close'):
    ...
    assert problem in ['close', 'VWAP', 'index', 'industry', 'VWAP_day20', 'close_day20']

    if problem == 'close':
        result = ((row['close'] - row['close_lag']) / row['close_lag'] if row['close_lag'] != 0 else 0) * 100
        
    elif (problem == 'VWAP') or (problem == 'VWAP_day20'):
...
def label(row):
    ...
    future_return = row['close_after'] - row['close']
    if row['index_return'] < 0:
        ...
        if future_return > row['index_return']:
            return 1
        else:
            return 0


def get_label(data, forward, look_back_month):

    d = data.sort_values(by='ts').reset_index(drop=True)
    d['index_high_month'] = d['index_high'].rolling(window=forward).max()
    d['index_low_month'] = d['index_low'].rolling(window=forward).min()
    d['index_close_after'] = d['index_close'].shift(-forward)
    d['index_open_month'] = d['index_open'].shift(forward)
    d['industry_high_month'] = d['industry_high'].rolling(window=forward).max()
    d['industry_low_month'] = d['industry_low'].rolling(window=forward).min()
    d['industry_open_month'] = d['industry_open'].shift(forward)
    d['index_return'] = (d['index_close_after'] - d['index_close']) /d['index_close']
    d['close_after'] = d['close'].shift(-forward)
    d['Y_TR'] = d.apply(label, axis=1)
...
def get_features(data, columns_dict, look_back, forward):
    ...
    for col in lag:
        d[f'{col}_lag'] = d[col].shift(forward)
    ...
    for buy in buyer:
        ...
        d[f'{buy}_4week_ratio'] = d[buy].rolling(window=20, min_periods=1).sum() / d['vol'].rolling(window=20, min_periods=1).sum()
