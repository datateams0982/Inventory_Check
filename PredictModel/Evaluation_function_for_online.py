...
def read_prediction(file_path, origin_path):
    ...
    prediction = pd.read_csv(file_path, converters={'StockNo': str, 'ts': str})
    prediction['ts'] = pd.to_datetime(prediction['ts'])

    original = pd.read_csv(origin_path, converters={'StockNo': str, 'ts': str}, usecols=['ts', 'StockNo', 'close', 'VWAP_day5', 'VWAP', 'Y', 'VWAP_after', 'index_close', 'industry_close', 'foreign_ratio', 'investment_ratio', 'corporation_ratio', 'PctRank'])
    original['ts'] = pd.to_datetime(original['ts'])
    original['future_return'] = original['VWAP_after'] - original['VWAP_day5']
...
def verification(prediction):

    d = prediction[prediction.future_return.notnull()]
...
def daily_evaluation(prediction, threshold=0.5):
...
def Stock_evaluation(prediction, threshold=0.5):
...
def monthly_evaluation(prediction, threshold=0.5):
    ...
    df = pd.DataFrame(columns=['accuracy', 'return', 'ratio', 'portfolio', 'up_precision', 'down_precision', 'up_support_mean', 'down_support_mean', 'index'], index=index_list)
