...
class FeatureEngineering:
    ...
    def __init__(self, df_path, save_path):
        ...
        self._df = pd.read_csv(df_path, converters={'ts': str, 'StockName': str, 'StockNo': str})
    ...
    def Engineering(self):
        ...
        self._df_list = [group[1] for group in self._df.groupby(self._df['StockNo'])]

        self._output = []

        with Pool(processes=12) as pool:
            for i, x in enumerate(tqdm(pool.imap_unordered(get_technical_indicators, self._df_list), total=len(self._df_list)), 1):
                self.output.append(x)

        self.final = pd.concat(self._output, axis=0)

    def save_data(self):

        self.final.to_csv(self._path, index=False)