...
def log_return(row, problem='close'):
    ...
    if problem == 'close':
        ...
        if row[f'{problem}_lag'] == 0:
            result = 0
        else:
            result = ((row[problem] - row[f'{problem}_lag']) / row[f'{problem}_lag']) * 100
...
def get_technical_indicators(data, columns_dict, look_back=15, forward=5):
    ...
    d[f'VWAP_day{forward}'] = d['total'].rolling(window=forward).sum()/d['vol'].rolling(window=forward).sum()
    ...
    for i in [5, 10, 15]:
        for col in ratio_col:
            d[f'{col}_ratio_day{i}'] = d[col]/ d[col].rolling(window=i).mean()
    ...
    for buy in buyer:
        ...
        d[f'{buy}_day3_ratio'] = d[buy].rolling(window=3).sum() / d['vol'].rolling(window=3).sum()
        d[f'{buy}_week_ratio'] = d[buy].rolling(window=5).sum() / d['vol'].rolling(window=5).sum()
        d[f'{buy}_2week_ratio'] = d[buy].rolling(window=10).sum() / d['vol'].rolling(window=10).sum()
        d[f'{buy}_3week_ratio'] = d[buy].rolling(window=15).sum() / d['vol'].rolling(window=15).sum()
    ...
    d = STO(d, nk=15, nD=5, forward=forward)
    d = d.replace([np.inf, -np.inf], 0)
    d = d[d.ts.dt.date < date(2019,9,23)]

    d = d.drop(columns=['VWAP_lag', 'VWAP_after', 'ema_20', 'ema_5', 'close_lag', 'index_close_lag', 'industry_close_lag'])
