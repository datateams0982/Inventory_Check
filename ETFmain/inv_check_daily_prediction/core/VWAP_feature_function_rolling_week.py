...
def price_volume_trend(df, rolling_window):
    ...
    df_pvt[f'index_pvt_current'] = df_pvt[f'index_vol'] * (df_pvt[f'index_close'] - df_pvt[f'index_close'].shift(1)) / df_pvt[f'index_close'].shift(1)
    ...
    feature = [f'{item}pvt_current' for item in ['index_', 'foreign_', 'investment_', 'dealer_', '']]
    pvt_feature = [f'{item}pvt' for item in ['index_', 'foreign_', 'investment_', 'dealer_', '']]
...
def log_return(row, problem='close'):
    ...
    assert problem in ['close', 'VWAP', 'index', 'VWAP_day5']
...
def get_features(data, columns_dict, look_back=15, forward=5):
    ...
    d = d.drop(columns=['VWAP_lag', 'close_lag', 'index_close_lag'])
...
def read_feature_list(file_path, requirement='whole'):
    ...
    assert requirement in ['whole', 'technical', 'chip', 'index']

    d = []
    with open(file_path, 'r') as fp:
        d = json.load(fp)
    
    feature_list = d[f'ETF_{requirement}']
